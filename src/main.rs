mod parser;
pub mod colors; use colors as color;
...
fn main() {
    ...
    let inp = args
        .next()
        .die(&format!("{}ERROR: Please, provide a valid input file name.{}", color::RED_BOLD, color::DEFAULT));
    let out_s = args
        .next()
        .die(&format!("{}ERROR: Please, provide an output file name.{}", color::RED_BOLD, color::DEFAULT));
    let debug = args
        .next()
        .unwrap_or("".into());
    let debug = if debug == "--debug" { true }
                     else { false };
